...
dominators g =
    let (0, len) = A.bounds g in
    let tree = IM.fromList [ (n, IS.fromList (g A.! n)) | n <- [0..len] ] in
    let res_dom = sort $ dom (1, tree) in
    let res_map = M.fromList res_dom in
    let res_idom = sort $ idom (1, tree) in
    -- DF(n) = all nodes y such that n dominates a direct predecessor of y but
    --         does not strictly dominate y
    let a `d` b = a == b || case res_map M.!? b of
                                Nothing -> False
                                Just list -> elem a list in
    let a `sd` b = (a `d` b) && a /= b in
    let pred n = nub [ a | (a, b) <- G.edges g, n == b ] in
    let dominance_frontier n =
            [ y | y <- [0..len],
                any (\z -> n `d` z) (pred y) && not (n `sd` y) ] in
    let frontier = [ (n, dominance_frontier n) | n <- [0..len] ] in
    let dom_tree = [ (n, [ x |
                          (x, d) <- res_idom, d == n, x /= 1 ]) |
                      n <- [0..len] ] in
    (res_dom, res_idom, dom_tree, frontier)
